...
describe('test fetch ref', () => {
    ...
    test('test ref with trailing slash', () => {
        const rec = cp_obj.fetchComponentByRef('#/components/x-bte-kgs-operations/enablesMF/')
        expect(rec[0]['source']).toBe('entrez');
    });

    test("test wrong ref", () => {
        const rec = cp_obj.fetchComponentByRef("/components/x-bte-response-mapping");
